...
{
    internal class ServiceEntityGenerator : EntityGenerator
    {
        private readonly string _syncObjectSchema;
        ...
        ///     1. An XXXEntities.cs file that contains the SyncScope attributed collection name and all the individual Entities
        ///     that make that scope.
        ...
        {
            var cc = GenerateEntitiesCompileUnit(filePrefix, nameSpace, desc, colsMappingInfo);

            CodeDomUtility.SaveCompileUnitToFile(cc, option,
            ...
            var codeBehindFilename = CodeDomUtility.GenerateFileName(desc.ScopeName, dirInfo, filePrefix,
                "SyncService.svc", option);
            CodeDomUtility.SaveCompileUnitToFile(cc, option, codeBehindFilename);

            // Generate the actual .SVC file.
            CodeDomUtility.SaveSVCFile(nameSpace, cc.Namespaces[0].Types[0].Name, codeBehindFilename,
                CodeDomUtility.GenerateFileName(desc.ScopeName, dirInfo, filePrefix, "SyncService", CodeLanguage.SVC),
                option);
        }

        private CodeCompileUnit GenerateSyncServiceCompileUnit(string prefix, string nameSpace,
            DbSyncScopeDescription desc)
        {
            ...
            var wrapperEntity = new CodeTypeDeclaration(
                string.Format(Constants.ServiceCustomSyncServiceTypeFormat,
                    string.IsNullOrEmpty(prefix) ? desc.ScopeName : prefix)
                );

            var baseSyncType = new CodeTypeReference(Constants.ServiceSyncServiceBaseTypeFormat);
            baseSyncType.TypeArguments.Add(string.Format(Constants.ServiceOuterEntityNameFormat,
                string.IsNullOrEmpty(prefix) ? desc.ScopeName : prefix));
            wrapperEntity.BaseTypes.Add(baseSyncType);

            wrapperEntity.StartDirectives.Add(new CodeRegionDirective(CodeRegionMode.Start,
                "SyncService: Configuration and setup"));

            // Add the method InitializeService
            var initSvcMethod = new CodeMemberMethod();
            initSvcMethod.Name = "InitializeService";
            initSvcMethod.Attributes = MemberAttributes.Public | MemberAttributes.Static;
            initSvcMethod.Parameters.Add(
                new CodeParameterDeclarationExpression(
                    new CodeTypeReference(Constants.ServiceISyncSvcConfigurationType), "config"));

            // Add comments for the following lines.
            foreach (var s in Constants.ServiceSyncSvcCommentLines)
            {
            ...
            {
                foreach (var s in Constants.ServiceSyncSvcPreSchemaCommentLines)
                {
                    initSvcMethod.Statements.Add(new CodeCommentStatement(s));
                }
                ...
                var expr =
                    new CodeMethodInvokeExpression(new CodeVariableReferenceExpression("config"),
                        ...
                        new CodePrimitiveExpression(_syncObjectSchema));
        ...
        {
            return (colsMapping == null || !colsMapping.ContainsKey(columnName.ToLowerInvariant()))
                ? columnName
                : colsMapping[columnName.ToLowerInvariant()];
        }
    }
}