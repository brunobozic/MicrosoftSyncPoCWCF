...
namespace Microsoft.Synchronization.ClientServices.CodeDom
{
    ...
    {
        public static CodeTypeDeclaration GetEntityForTableDescription(DbSyncTableDescription tableDesc,
            bool addKeyAttributes, Dictionary<string, string> colsMapping)
        {
            var entityDeclaration = new CodeTypeDeclaration(SanitizeName(tableDesc.UnquotedGlobalName));
            entityDeclaration.IsPartial = true;
            entityDeclaration.IsClass = true;

            foreach (var column in tableDesc.Columns)
            {
                var colName = column.UnquotedName;
                if (colsMapping != null)
                {
                    colsMapping.TryGetValue(column.UnquotedName.ToLowerInvariant(), out colName);
                    colName = colName ?? column.UnquotedName;
                }
                var fieldTypeReference = GetTypeFromSqlType(tableDesc, column);
                var colField = new CodeMemberField(fieldTypeReference, "_" + SanitizeName(colName));
                ...
                propertyField.GetStatements.Add(
                    new CodeMethodReturnStatement(new CodeArgumentReferenceExpression(colField.Name)));
                propertyField.SetStatements.Add(
                    new CodeAssignStatement(new CodeVariableReferenceExpression(colField.Name),
                ...
                else
                {
                    ...
                    {
                        propertyField.CustomAttributes.Add(
                            new CodeAttributeDeclaration(Constants.ServiceSyncColumnMappingAttribute,
                                new CodeAttributeArgument("LocalName",
                                    new CodeSnippetExpression("\"" + column.UnquotedName + "\""))));
                    }
                    ...
                    {
                        propertyField.CustomAttributes.Add(
                            new CodeAttributeDeclaration(Constants.EntityPropertyNullableAttributeType));
                    }
                }
        ...
        {
            var csprovider = CodeDomProvider.CreateProvider(option.ToString());
        ...
        public static void SaveSVCFile(string ns, string syncSvcTypeName, string codeBehindFileName, string fileName,
            CodeLanguage option)
        {
            ...
            builder.AppendFormat(Constants.ServiceSyncServicSVCFileContents, option.ToString().ToLowerInvariant(), ns,
                ...
                AssemblyName.GetAssemblyName(Assembly.GetCallingAssembly().Location).Version);
            using (var fs = new FileStream(fileName, FileMode.Create, FileAccess.Write))
            {
                using (var writer = new StreamWriter(fs))
                {
        ...
        {
            var baseDeclaration =
                new CodeTypeDeclaration(string.Format(Constants.OfflineEntityBaseTypeNameFormat, baseNamePrefix));
            ...
            ctor.Statements.Add(
                new CodeAssignStatement(
                    ...
                    new CodeObjectCreateExpression(isServer
                        ? Constants.ServiceOfflineEntityMetadataTypeName
                        : Constants.ClientOfflineEntityMetadataTypeName)));
            baseDeclaration.Members.Add(ctor);

            // Add the ServiceMetadata field            
            var svcMetadataField = new CodeMemberField();
            svcMetadataField.Attributes = MemberAttributes.Private;
            svcMetadataField.Name = Constants.ServiceMetadataFieldName;
            svcMetadataField.Type =
                new CodeTypeReference(isServer
                    ? Constants.ServiceOfflineEntityMetadataTypeName
                    : Constants.ClientOfflineEntityMetadataTypeName);
            baseDeclaration.Members.Add(svcMetadataField);

            // Add the ServiceMetadata Property            
            var svcMetadataProp = new CodeMemberProperty();
            svcMetadataProp.Attributes = MemberAttributes.Public;
            svcMetadataProp.Name = Constants.ServiceMetadataPropertyName;
            svcMetadataProp.Type =
                new CodeTypeReference(isServer
                    ? Constants.ServiceOfflineEntityMetadataTypeName
                    : Constants.ClientOfflineEntityMetadataTypeName);
            svcMetadataProp.ImplementationTypes.Add(isServer
                ? new CodeTypeReference(Constants.ServiceIOfflineEntity)
                : new CodeTypeReference(Constants.ClientIOfflineEntity));
            svcMetadataProp.GetStatements.Add(
                new CodeMethodReturnStatement(new CodeArgumentReferenceExpression(svcMetadataField.Name)));

            // This creates a check for following.
            // if(value == null) throw new ArgumentNullException("value")
            var checkForNullStmt = new CodeConditionStatement(
                new CodeBinaryOperatorExpression(
                    new CodeSnippetExpression("value"), CodeBinaryOperatorType.IdentityEquality,
            ...
            svcMetadataProp.SetStatements.Add(
                new CodeAssignStatement(new CodeVariableReferenceExpression(svcMetadataField.Name),
                    new CodeVariableReferenceExpression("value")));
        ...
        private static CodeTypeReference GetTypeFromSqlType(DbSyncTableDescription tableDesc,
            DbSyncColumnDescription colDesc)
        {
            var sqltype = colDesc.Type;
            var isNullable = colDesc.IsNullable;
            ...
            switch (type)
            {
                ...
                default:
                    throw new NotSupportedException(
                        string.Format("Column '{0}' in Table '{1}' has an unsupported SqlType - '{2}'",
                            colDesc.UnquotedName, tableDesc.UnquotedGlobalName, sqltype));
            }

            if (isNullable && retType.IsValueType)
            {
                var ctr = new CodeTypeReference(typeof (Nullable<>));
