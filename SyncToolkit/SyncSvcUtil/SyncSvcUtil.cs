...
namespace Microsoft.Synchronization.ClientServices
{
    ...
    public class SyncSvcUtil
    {
        ...
        private static void ProcessCSDLUri(ArgsParser parser)
        {
            ...
            var scopeDescription = CSDLParser.GetDescriptionFromUri(parser, parser.CSDLUrl, out serviceUri);

            Log("Generating files...");
            var generator = EntityGeneratorFactory.Create(parser.CodeGenMode, null
                /* null syncSchema - not needed for client code generation */);
        ...
        private static void ProcessConfigFile(ArgsParser parser)
        {
            ...
            var config =
                ConfigurationManager.OpenMappedExeConfiguration(
                    new ExeConfigurationFileMap {ExeConfigFilename = parser.ConfigFile}, ConfigurationUserLevel.None);
        ...
        private static Dictionary<string, Dictionary<string, string>> BuildColumnMappingInfo(
            SelectedConfigSections selectedConfig)
        {
            var mappingInfo = new Dictionary<string, Dictionary<string, string>>();
        ...
        private static void CheckFilterParamTypeAndSize(FilterParameterConfigElement filterParam)
        {
            ...
            if (!filterParam.Name.Trim().StartsWith("@", StringComparison.Ordinal))
            {
                throw new ConfigurationErrorsException(
                    string.Format("FilterParameter '{0}' Name property does not start with '@'.", filterParam.Name));
            }

            // First check that SqlType is a valid SqlDbType enum
            if (
                Enum.GetNames(typeof (SqlDbType))
                    .Where(e => e.Equals(filterParam.SqlType, StringComparison.OrdinalIgnoreCase))
                    .Count() == 0)
            {
                throw new ConfigurationErrorsException(
                    string.Format("SqlType '{0}' for filter parameter {1} is not a valid SqlDbType enum.",
                        filterParam.SqlType, filterParam.Name));
            }

            // Ensure that the DataSize attribute has a value set for the required types
            var type = (SqlDbType) Enum.Parse(typeof (SqlDbType), filterParam.SqlType, true);
            switch (type)
            {
                ...
                case SqlDbType.VarChar:
                    if (filterParam.DataSize <= 0)
                    {
                        throw new ConfigurationErrorsException(
                            string.Format(
                                "Filter parameter '{0}' must specify a non zero number for the DataSize attribute.",
                                filterParam.Name));
                    }
                    break;
            }
        ...
        /// <summary>
        ///     Based on the passed in SyncScope element and the TargetDatabase element the system generates a
        ///     DbSyncScopeDescription element.
        /// </summary>
        ...
        {
            var desc = new DbSyncScopeDescription(selectedConfig.SelectedSyncScope.Name);

            using (var conn = new SqlConnection(selectedConfig.SelectedTargetDatabase.GetConnectionString()))
            {
                conn.Open();
                foreach (SyncTableConfigElement table in selectedConfig.SelectedSyncScope.SyncTables)
                {
                    var tableDesc = SqlSyncDescriptionBuilder.GetDescriptionForTable(table.Name, conn);
                    ...
                    {
                        if (
                            tableDesc.Columns.Where(
                                e => e.UnquotedName.Equals(colElem.Name, StringComparison.OrdinalIgnoreCase))
                                .FirstOrDefault() == null)
                        {
                            throw new InvalidOperationException(
                                string.Format(
                                    "Table '{0}' does not have a column '{1}' defined in the target database. Please check your SyncColumn definitions",
                                    table.Name, colElem.Name));
                        }
                    }

                    var columnsToRemove = new List<DbSyncColumnDescription>();
                    ...
                    if (!table.IncludeAllColumns || table.SyncColumns.Count > 0)
                    {
                        ...
                        foreach (var columnDesc in tableDesc.Columns)
                        {
                            var configElement =
                                table.SyncColumns.Cast<SyncColumnConfigElement>()
                                    .FirstOrDefault(
                                        e =>
                                            e.Name.Equals(columnDesc.UnquotedName,
                    ...
                    if (tableDesc.Columns.Count == 0)
                    {
                        throw new InvalidOperationException(
                            string.Format(
                                "SyncTable '{0}' has zero SyncColumns configured for sync. Either set IncludeAllColumns to true or specify atleast one SyncColumn.",
                                table.Name));
                    }
        ...
        {
            var sections = new SelectedConfigSections();
            ...
            {
                sections.SelectedSyncScope =
                    syncConfig.SyncScopes.Cast<SyncScopeConfigElement>()
                        .Single(e => e.Name.Equals(parser.ScopeName, StringComparison.InvariantCultureIgnoreCase));
            }
            ...
            {
                sections.SelectedTargetDatabase =
        ...
        private static void ValidateCSDLMode(ArgsParser parser)
        {
            ...
            {
                throw new InvalidOperationException(
                    "/target:server is not a valid target when /url flag is used. Only isclient and client targets allowed.");
            }
        }
    }

    internal class SelectedConfigSections
    {
