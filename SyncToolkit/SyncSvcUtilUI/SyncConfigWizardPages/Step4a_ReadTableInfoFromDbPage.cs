...
namespace SyncSvcUtilUI.SyncConfigWizardPages
{
    ...
    {
        private const string AndFilterClauseFormat = " AND [side].{0} = {1}";
        private const string FilterClauseFormat = "[side].{0} = {1}";
        private readonly SyncScopeConfigElement selectedScope;
        private DbSyncTableDescription tableDesc;
        ...
        private void Init()
        {
            ...
            {
                dbsComboBox.Items.Add(db.Name);
            }

            dbsComboBox.SelectedIndex = -1;
        }
        ...
        private void dbsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var conn = new SqlConnection(db.GetConnectionString()))
                {
                    ...
                    var cmd = new SqlCommand(WizardHelper.SELECT_TABLENAMES_QUERY, conn);
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            var fullName = string.Format("{0}.{1}", schema, tableName);
                            var quotedTableName = string.Format("[{0}]", tableName);
                            var quotedFullName = string.Format("[{0}].[{1}]", schema, tableName);

                            if (IsTableNameValid(tableName))
                            {
                                if (schema.Equals("dbo", StringComparison.OrdinalIgnoreCase))
                                {
                                    tablesBox.Items.Add(quotedTableName,
                                        (selectedScope.SyncTables.GetElement(quotedTableName) != null ||
                                         selectedScope.SyncTables.GetElement(tableName) != null));
                                }
                                else
                                {
                                    tablesBox.Items.Add(quotedFullName,
        ...
        private void tablesBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (table != null)
            {
                ...
                tableDesc = SqlSyncDescriptionBuilder.GetDescriptionForTable(table.Name,
                    new SqlConnection(db.GetConnectionString()));
