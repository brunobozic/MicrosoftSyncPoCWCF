...
namespace SyncSvcUtilUI.CodegenWizardPages
{
    public partial class Step1_PickConfigOrCSDLModelPage : UserControl, IWizardPage
    {
        ...
        private void openBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                openCfgTextBox.Text = openFileDialog.FileName;
                ...
                WizardHelper.Instance.SyncConfigSection =
                    (SyncConfigurationSection)
                        ((SyncConfigurationSection) config.GetSection("SyncConfiguration")).Clone();
                ...
                {
                    scopesBox.Items.Add(scope.Name);
                }

                // Populate target dbs name
                foreach (TargetDatabaseConfigElement db in WizardHelper.Instance.SyncConfigSection.Databases)
                {
                    dbsBox.Items.Add(db.Name);
                }

                scopesBox.SelectedIndex = (scopesBox.Items.Count == 1) ? 0 : -1;
                dbsBox.SelectedIndex = (dbsBox.Items.Count == 1) ? 0 : -1;
            }
        }
        ...
        private void browseBtn_Click(object sender, EventArgs e)
        {
            ...
            csdlScopesBox.Items.Clear();

            try
            {
                var wc = new WebClient();
                wc.DownloadStringCompleted += wc_DownloadStringCompleted;
                wc.DownloadStringAsync(new Uri(csdlUrl.Text));
                openBtn.Enabled = false;
                csdlUrl.Enabled = false;
        ...
        private void wc_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            csdlUrl.Enabled = true;
            XNamespace ns = "http://www.w3.org/2007/app";
            // Success
            var document = XDocument.Parse(e.Result);
            var workspaceElem = document.Root.Element(ns + "workspace");

            if (workspaceElem != null)
            {
                foreach (var elem in workspaceElem.Elements(ns + "collection"))
                {
                    if (elem.Attribute("href") != null)
                    {
                        csdlScopesBox.Items.Add(elem.Attribute("href").Value);
                    }
                }
            }
        ...
        {
            browseBtn.Enabled = csdlUrl.Text.Length > "http://".Length;
        }

        private void useConfigOption_CheckedChanged(object sender, EventArgs e)
        {
            configPanel.Enabled = useConfigOption.Checked;
            csdlPanel.Enabled = csdlOption.Checked;
        }
        ...
        public void OnFocus()
        {
            openCfgTextBox.Focus();
        }

        public bool OnMovingNext()
        {
            if (useConfigOption.Checked)
            {
                ...
                {
                    MessageBox.Show("Please specify a SyncConfig file name.", "Error", MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return false;
                }

                // Check that atleast on Scope has been selected
                if (scopesBox.SelectedIndex == -1)
                {
                ...
                if (dbsBox.SelectedIndex == -1)
                {
                    MessageBox.Show("Please specify a Target Database before proceeding.", "Error", MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return false;
                }

                WizardHelper.Instance.CodeGenWizardHelper[WizardHelper.SELECTED_DB_NAME] =
                    dbsBox.SelectedItem.ToString();
                WizardHelper.Instance.CodeGenWizardHelper[WizardHelper.SELECTED_CONFIG_NAME] =
                    scopesBox.SelectedItem.ToString();
                WizardHelper.Instance.CodeGenWizardHelper[WizardHelper.SELECTED_CODEGEN_SOURCE] =
                    WizardHelper.CONFIG_FILE_CODEGEN_SOURCE;
            }
            else
            {
                // Check that atleast on Scope has been selected
                if (csdlScopesBox.SelectedIndex == -1)
                {
                    MessageBox.Show("Please specify a SyncScope before proceeding.", "Error", MessageBoxButtons.OK);
                    return false;
                }
                ...
                WizardHelper.Instance.CodeGenWizardHelper[WizardHelper.SELECTED_CONFIG_NAME] =
                    csdlScopesBox.SelectedItem.ToString();
                WizardHelper.Instance.CodeGenWizardHelper[WizardHelper.SELECTED_CODEGEN_SOURCE] =
                    WizardHelper.CSDL_CODEGEN_SOURCE;
                WizardHelper.Instance.CodeGenWizardHelper[WizardHelper.CSDL_CODEGEN_URL] =
                    string.Format(WizardHelper.CSDL_CODEGEN_URL_FORMAT, csdlUrl.Text, csdlScopesBox.SelectedItem);
            }
